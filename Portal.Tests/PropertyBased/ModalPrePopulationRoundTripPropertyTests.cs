...
// Feature: quotation-edit-modal-lines, Property 3: Edit button populates modal fields matching data attributes

/// <summary>
/// Property-based tests for modal pre-population round-trip integrity.
/// Validates that the serialization of QuotationLine field values to data-* attributes
...
/// </summary>
public class ModalPrePopulationRoundTripPropertyTests
{
    #region Generators
    ...
    /// <summary>
    /// Generates a nullable ProposalSectionId (null or 1-1000).
    /// </summary>
    private static Gen<int?> NullableSectionIdGen()
    {
        return Gen.OneOf(
            Gen.Constant<int?>(null),
            Gen.Choose(1, 1000).Select(i => (int?)i)
        );
    }

    /// <summary>
    /// Generates a complete line item field value object for round-trip testing.
    /// </summary>
    private static Gen<LineItemFieldValues> LineItemFieldValuesGen()
    {
        return from description in DescriptionGen()
               ...
               from proposalSectionId in NullableSectionIdGen()
               select new LineItemFieldValues(
                   description, subtitle, referenceUrl,
                   quantity, unitPrice, vatRate,
                   discount, discountType, costPrice,
                   isReverseCharge, productCode, proposalSectionId);
    }
    ...
    /// This matches the Razor template patterns in _SectionCards.cshtml.
    /// </summary>
    private static DataAttributeStrings SerializeToDataAttributes(LineItemFieldValues fields)
    {
        return new DataAttributeStrings(
            ...
            ProposalSectionId: fields.ProposalSectionId?.ToString() ?? ""
        );
    }

    /// <summary>
    /// Simulates JavaScript reading data attributes and populating form fields (step 2: string → parsed values).
    /// This matches the JS logic in quotation-line-modal.js showEditLineModal().
    /// </summary>
    private static LineItemFieldValues DeserializeFromDataAttributes(DataAttributeStrings attrs)
    {
        return new LineItemFieldValues(
            ...
            ProposalSectionId: int.TryParse(attrs.ProposalSectionId, out var sectionId) ? sectionId : null
        );
    }
    ...
    /// <summary>
    /// Property 3: For any line item with arbitrary field values, the data attribute
    ...
    public Property DataAttribute_RoundTrip_PreservesAllFieldValues()
    {
        return Prop.ForAll(
            LineItemFieldValuesGen().ToArbitrary(),
            original =>
            {
                ...
                var sectionIdMatch = roundTripped.ProposalSectionId == original.ProposalSectionId;
                ...
                var allMatch = descriptionMatch && subtitleMatch && referenceUrlMatch &&
                               ...
                               reverseChargeMatch && productCodeMatch && sectionIdMatch;
                ...
                return allMatch
                    ...
                    .And(sectionIdMatch.Label($"ProposalSectionId: expected={original.ProposalSectionId}, actual={roundTripped.ProposalSectionId}"));
            });
    }
    ...
    public Property DataAttribute_NullableFields_EmptyStringMapsToNull()
    {
        return Prop.ForAll(
            ...
            original =>
            {
                ...
                var sectionIdCorrect = original.ProposalSectionId == null
                    ? dataAttributes.ProposalSectionId == ""
                    : dataAttributes.ProposalSectionId == original.ProposalSectionId.Value.ToString();
                ...
                return subtitleCorrect
                    ...
                    .And(sectionIdCorrect.Label($"ProposalSectionId null→empty: original={original.ProposalSectionId}, attr='{dataAttributes.ProposalSectionId}'"));
            });
    }
    ...
    /// <summary>
    /// Represents all field values of a line item entity.
    /// </summary>
    private record LineItemFieldValues(
        string Description,
        string? Subtitle,
        string? ReferenceUrl,
        decimal Quantity,
        ...
        string? ProductCode,
        int? ProposalSectionId);

    /// <summary>
    /// Represents the string values stored in data-* attributes on a table row.
    /// </summary>
    private record DataAttributeStrings(
        string Description,
        string Subtitle,
        string ReferenceUrl,
        string Quantity,
