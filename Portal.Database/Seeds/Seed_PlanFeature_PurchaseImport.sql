-- ============================================================
-- Seed: Adds the 'purchase_import' module to Professional and
...
IF @ProfessionalPlanId IS NOT NULL
   AND NOT EXISTS (
       ...
       WHERE [PlanId] = @ProfessionalPlanId AND [ModuleName] = N'purchase_import'
   )
BEGIN
    INSERT INTO [dbo].[PlanFeature] ([PlanId], [ModuleName], [IsIncluded], [AccessLevel])
    VALUES (@ProfessionalPlanId, N'purchase_import', 1, N'full');
END
GO
...
IF @EnterprisePlanId2 IS NOT NULL
   AND NOT EXISTS (
       SELECT 1 FROM [dbo].[PlanFeature]
       WHERE [PlanId] = @EnterprisePlanId2 AND [ModuleName] = N'purchase_import'
   )
BEGIN
    INSERT INTO [dbo].[PlanFeature] ([PlanId], [ModuleName], [IsIncluded], [AccessLevel])
    VALUES (@EnterprisePlanId2, N'purchase_import', 1, N'full');
END
GO
