-- ============================================================
-- Seed: Adds the 'attachments' module to Professional and
...
IF @ProfessionalPlanId IS NOT NULL
   AND NOT EXISTS (
       ...
       WHERE [PlanId] = @ProfessionalPlanId AND [ModuleName] = N'attachments'
   )
BEGIN
    INSERT INTO [dbo].[PlanFeature] ([PlanId], [ModuleName], [IsIncluded], [AccessLevel])
    VALUES (@ProfessionalPlanId, N'attachments', 1, N'full');
END
GO
...
IF @EnterprisePlanId2 IS NOT NULL
   AND NOT EXISTS (
       SELECT 1 FROM [dbo].[PlanFeature]
       WHERE [PlanId] = @EnterprisePlanId2 AND [ModuleName] = N'attachments'
   )
BEGIN
    INSERT INTO [dbo].[PlanFeature] ([PlanId], [ModuleName], [IsIncluded], [AccessLevel])
    VALUES (@EnterprisePlanId2, N'attachments', 1, N'full');
END
GO
