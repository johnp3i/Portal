...
public class PortalEmailService : IEmailService
{
    ...
    public PortalEmailService(IEmailSender emailSender, ILogger<PortalEmailService> logger)
    {
    ...
    public async Task SendProposalEmailAsync(string toEmail, string shareToken, string quotationReference, string businessName, DateTimeOffset expiresAtUtc)
    {
        try
        {
            ...
            var htmlBody = BuildProposalHtml(shareToken, quotationReference, businessName, expiresAtUtc);
...
<body style=""margin:0;padding:0;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background-color:#F2F6FA;"">
    ...
    private static string BuildProposalHtml(string shareToken, string quotationReference, string businessName, DateTimeOffset expiresAtUtc)
    {
        var proposalUrl = $"/proposal/{System.Net.WebUtility.UrlEncode(shareToken)}";
                ...
                <table role=""presentation"" width=""600"" cellpadding=""0"" cellspacing=""0"" border=""0"" style=""max-width:600px;width:100%;background-color:#FFFFFF;border-radius:16px;overflow:hidden;"">
                    ...
                    <tr>
                        <td style=""padding:40px;"">
                            ...
                            <p style=""margin:28px 0 0 0;"">
                                <a href=""{proposalUrl}"" style=""display:inline-block;padding:14px 32px;background-color:#0D5EA6;color:#FFFFFF;font-size:16px;font-weight:700;text-decoration:none;border-radius:8px;"">View Proposal</a>
                            </p>
                            <p style=""margin:24px 0 0 0;font-size:13px;color:#5E7385;line-height:1.6;"">
