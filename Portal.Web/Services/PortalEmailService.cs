...
<body style=""margin:0;padding:0;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;background-color:#F2F6FA;"">
    ...
    public async Task SendPayslipEmailAsync(string toEmail, string employeeName, string businessName, string monthName, int year, byte[] pdfBytes, string filename)
    {
        try
        {
            var subject = $"Your Payslip - {monthName} {year}";
            var htmlBody = BuildPayslipEmailHtml(employeeName, businessName, monthName, year);

            await _emailSender.SendEmailWithAttachmentAsync(toEmail, subject, htmlBody, EmailDepartmentEnum.Payroll, pdfBytes, filename, "application/pdf");

            _logger.LogInformation("Payslip email sent to {Email} for {EmployeeName} ({MonthName} {Year})", toEmail, employeeName, monthName, year);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to send payslip email to {Email} for {EmployeeName}", toEmail, employeeName);
            throw;
        }
    }

    private static string BuildPayslipEmailHtml(string employeeName, string businessName, string monthName, int year)
    {
        var encodedEmployeeName = System.Net.WebUtility.HtmlEncode(employeeName);
                    ...
                    <tr>
                        <td style=""padding:40px;"">
                            <table role=""presentation"" cellpadding=""0"" cellspacing=""0"" border=""0"">
                                <tr>
                                    <td style=""background-color:#EBF5FF;border-radius:20px;padding:6px 16px;"">
                                        <span style=""font-size:12px;font-weight:700;color:#0D5EA6;letter-spacing:0.06em;text-transform:uppercase;"">Payslip</span>
                                    </td>
                                </tr>
                            </table>
                            <h1 style=""margin:24px 0 0 0;font-size:24px;font-weight:700;color:#0B1B28;line-height:1.3;"">Your Payslip</h1>
                            <p style=""margin:16px 0 0 0;font-size:16px;line-height:1.7;color:#3D4F5F;"">
                                Hi {encodedEmployeeName},
                            </p>
                            <p style=""margin:16px 0 0 0;font-size:16px;line-height:1.7;color:#3D4F5F;"">
                            ...
                            <p style=""margin:24px 0 0 0;font-size:13px;color:#5E7385;line-height:1.6;"">
                                Best regards,<br />{encodedBusinessName}
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td style=""background-color:#0B1B28;padding:28px 40px;text-align:center;"">
                            <p style=""margin:0 0 8px 0;font-size:14px;font-weight:700;color:#FFFFFF;"">{encodedBusinessName}</p>
                            <p style=""margin:0;font-size:11px;color:#5E6D7A;letter-spacing:0.08em;"">Knowledge &middot; Professionalism &middot; Innovation</p>
                        </td>
                    </tr>
                </table>
